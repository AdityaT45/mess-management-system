...
export default function AdminProfile() {
    ...
    const loadProfile = useCallback(async () => {
        ...
        setError(null);
        try {
            const res = await adminService.getOwnMess();
            if (res.type === 'success') {
                setProfile(res.data);
            } else {
        ...
        <ScrollView contentContainerStyle={styles.scroll}>
            <View style={styles.card}>
                <View style={styles.headerRow}>
                    {profileImageUri ? (
                        ...
                        <View style={[styles.avatar, styles.avatarPlaceholder]}>
                            <Text style={styles.avatarInitial}>{(profile?.ownerName || 'A').charAt(0).toUpperCase()}</Text>
                        </View>
                    )}
                    <View style={styles.headerTextWrap}>
                        <Text style={styles.name}>{profile?.messName || '-'}</Text>
                        <Text style={styles.email}>{profile?.email || '-'}</Text>
                        <View style={styles.badgeRow}>
                            {profile?.messType ? (
                                <View style={[styles.badge, styles.badgePrimary]}>
                                    <Text style={styles.badgeText}>{profile.messType}</Text>
                                </View>
                            ) : null}
                ...
                <View style={styles.quickGrid}>
                    ...
                    <InfoMini label="Owner Name" value={profile.ownerName} />
                    <InfoMini label="Mess Number" value={profile?.messNumber ? `#${profile.messNumber}` : '-'} />
                    <InfoMini label="Mess Type" value={profile?.messType || '-'} />
                </View>
                ...
                {showDetails ? (
                    <View>
                        ...
                        <View style={styles.rowWrap}>
                            ...
                            <InfoItem label="Mess Type" value={profile?.messType} />
                        </View>
                        <InfoItem label="Description" value={profile?.description} full />
                    </View>
                ) : null}
